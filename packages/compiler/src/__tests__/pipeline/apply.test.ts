...
describe("Pipeline Phase 4: apply()", () => {
  it("should apply multiple transformations correctly", () => {
    ...
    //   return <h1>Submit</h1>;\n (26 chars, total 75)
    // }
    const source = `import { zintl } from "zintljs";
function App() {
  return <h1>Submit</h1>;
}`;
      ...
      imports: [
        {
          ...
          location: {
            ...
            end: source.indexOf("import { zintl }") + 'import { zintl } from "zintljs";'.length,
            line: 1,
            column: 0,
          },
        },
    ...
    expect(result.code).toContain('import { zintl, t } from "zintljs";');
    expect(result.code).toContain(
  ...
  it("should handle Vue SFC with existing setup script", () => {
    ...
    const plan: ResolvedPlan = {
      ...
      imports: [{ source: "zintljs", specifiers: ["t"], strategy: "new" }],
      rewrites: [],
      diagnostics: [],
    };
  ...
  it("should handle replace strategy when start === end in imports", () => {
    ...
    const result = apply(source, plan, mockLogger);
    expect(result.code).toBe('import { t } from "zintljs";console.log("hi");');
  });
  ...
  it("should handle merge strategy and insert space when trimmedBefore matches contentBeforeBrace", () => {
    ...
    expect(result.code).toBe('import {zintl, t } from "zintljs";');
  });
  ...
  it("should handle mixed strategies correctly", () => {
    ...
    const result = apply(source, plan, mockLogger);
    expect(result.code).toContain('import { t } from "zintljs";');
  ...
  it("should inject script wrapper in Vue/Svelte SFCs if missing", () => {
    ...
    expect(vueResult.code).toContain('import { t } from "zintljs";');
    ...
    expect(svelteResult.code).toContain('import { t } from "zintljs";');
    expect(svelteResult.code).toContain("</script>");
  });

  it("should handle merge strategy when closing brace is missing in location", () => {
    const source = `import * as z from "zintljs";`;
    const plan: ResolvedPlan = {
      prepends: [],
      imports: [
        {
