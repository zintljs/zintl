...
| **Option / facet overlap**     | `assetsTarget`, `virtualAssets` and `similarityThreshold` exist both as top-level options and on the facets that consume them. Convenient, and one concept spelled two ways. **This is the surface we most expect to revisit before 1.0.** See the precedence rule below. |
