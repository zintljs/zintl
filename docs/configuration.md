...
Vue components must use `<script setup>`. That is a Vue constraint rather than an Rsbuild one and applies on Vite too: a plain `<script>` compiles its template into a separate render function whose expressions resolve against the component instance, and the helpers Zintl injects into the script block are not in scope there, so the render throws `_ctx._t is not a function`.
