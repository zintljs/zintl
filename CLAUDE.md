...
vpr ready:examples # build 27 example packages → 309 contract e2e tests        (~2-5 min)
...
| `examples/`          | 27 workspace packages — 26 real apps (React/Vue/Svelte/vanilla × SPA/SSR/MPA, on Vite and Rsbuild) plus the `custom-facets` library. Not demos — the contract suite drives them through real browsers. Every app renders the same locale bar (`docs/examples-locale-bar.md`); the page under it is its own starter's. |
