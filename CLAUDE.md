...
- **One retry** (`retry: 1`), and read it as a failure. The intent is zero — every flake traced in this suite turned out to be a real defect — but the suite cannot currently survive at zero, so the config settles at one and says so (`tests/vitest.config.ts`). The consequence is that a green run is not automatically a clean one: a `(retry x1)` beside a checkmark is a test that failed on its first attempt, and every defect found this way was found by someone reading past the checkmark. If a test needs the retry to pass, treat that as a bug report, not flakiness.
