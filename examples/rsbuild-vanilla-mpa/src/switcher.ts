...
/**
 * The bar's markup, minus the locale controls — {@link setupSwitcher} fills
 * those in once the DOM exists.
 *
 * Returned as a string so a page can drop it straight into its own template,
 * which is how the vanilla examples build their DOM.
 */
export function localeBar(): string {
  return `
    <section id="header">
...
/**
 * Render the locale controls into `element` and report clicks through `onSwitch`.
 *
 * Buttons, not links, because this app switches locale *at runtime* — the
 * catalog is swapped in place and nothing navigates. The baked examples
 * (`vanilla-spa-i18n-baked`, `vanilla-mpa-baked-i18n`) render links for the
 * opposite reason.
 */
export function setupSwitcher(element: HTMLElement, onSwitch: (lang: string) => void) {
  const lang = new URLSearchParams(window.location.search).get("lang") || "en";

  element.innerHTML = LOCALES.map((l) => {
    const active = lang === l.id;
    return `
    <!-- @zintl-ignore -->
    <button type="button" data-lang="${l.id}" class="${active ? "active" : ""}"${
