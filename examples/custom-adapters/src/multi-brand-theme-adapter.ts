...
import type { ZintlAdapter } from "@zintl/compiler";

export function createMultiBrandThemeAdapter(): ZintlAdapter {
  async function getTranslations(locale: string, context: any) {
    context.logger.info(`MultiBrandThemeAdapter loading all brands for locale: ${locale}`);
  ...
  return {
    name: "multi-brand-theme-adapter",
    type: "content",
    priority: 30,
